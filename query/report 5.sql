...
round(cast(sum(ip.quantity *p.unit_price)*1.0/nullif (count(distinct i.invoiceno),0) as numeric),2) as "Average Invoice Value"
from employees e
join position po
...
where ip.quantity>0 and ed.employee_department='Sales'
group by e.employee_id,e.first_name,e.last_name,po.position,ed.employee_department
order by round(cast(sum(ip.quantity *p.unit_price) as numeric),2) DESC;